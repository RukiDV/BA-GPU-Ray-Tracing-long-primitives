...
void HelloVulkan::createDescriptorSetLayout()
{
    ...
    m_descSetLayoutBind.addBinding(0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
...
void HelloVulkan::updateDescriptorSet()
{
    ...
    VkDescriptorBufferInfo dbiHairs{m_hairsBuffer.buffer, 0, VK_WHOLE_SIZE};
    writes.emplace_back(m_descSetLayoutBind.makeWrite(m_descSet, 3, &dbiHairs));

    VkDescriptorBufferInfo dbiClusters{m_clustersBuffer.buffer, 0, VK_WHOLE_SIZE};
    writes.emplace_back(m_descSetLayoutBind.makeWrite(m_descSet, 4, &dbiClusters));
...
void HelloVulkan::createGraphicsPipeline()
{
    ...
    gpb.addAttributeDescriptions({
                                         {0, 0, VK_FORMAT_R32G32B32_SFLOAT, static_cast<uint32_t>(offsetof(VertexObj,
                                                                                                           pos))},
                                         {1, 0, VK_FORMAT_R32G32B32_SFLOAT, static_cast<uint32_t>(offsetof(VertexObj,
                                                                                                           nrm))},
                                         {2, 0, VK_FORMAT_R32G32B32_SFLOAT, static_cast<uint32_t>(offsetof(VertexObj,
                                                                                                           color))},
                                         {3, 0, VK_FORMAT_R32G32_SFLOAT,    static_cast<uint32_t>(offsetof(VertexObj,
                                                                                                           texCoord))},
                                 });
...
void HelloVulkan::loadModel(const std::string& filename, nvmath::mat4f transform)
{
    ...
    for (int i = 0; i < 2000; i++)
    {
        ...
        instance.transformIT = nvmath::transpose(nvmath::invert((instance.transform)));
        ...
        for (uint32_t j = 0; j < segments[i]; j++)
        {
            nvmath::vec3 p0 =
                    nvmath::vec3(vertices[pointIndex + j * 3], vertices[pointIndex + 1 + j * 3],
                                 vertices[pointIndex + 2 + j * 3]) / 7.0f;
            nvmath::vec3 p1 =
...
void HelloVulkan::createPostPipeline()
{
    ...
    pipelineGenerator.addShader(nvh::loadFile("spv/passthrough.vert.spv", true, defaultSearchPaths, true),
...
{
    VkWriteDescriptorSet writeDescriptorSets = m_postDescSetLayoutBind.makeWrite(m_postDescSet, 0,
...
//
nvvk::RaytracingBuilderKHR::BlasInput HelloVulkan::hairToVkGeometryKHR(uint32_t i)
{
    VkDeviceAddress dataAddress = nvvk::getBufferDeviceAddress(m_device, m_clustersAabbBuffer[i].buffer);
    ...
    VkAccelerationStructureBuildRangeInfoKHR offset{};
    offset.firstVertex = 0;
    offset.primitiveCount = m_clusters[i].count;
    offset.primitiveOffset = 0;
    offset.transformOffset = 0;
...
nvmath::mat4 HelloVulkan::calculateTransformation(const Cluster& cluster)
{
...
//    }
    ...
    return {(-v.z * v.z - v.y * v.y) * safe + 1.0f, v.x * v.y * safe + v.z,
...
float HelloVulkan::calculateAabb(Aabb& aabb, const nvmath::mat4& trans, const uint32_t i)
{
    nvmath::mat4f transInv = nvmath::invert(trans);
    float surface = 0;
//    for (uint32_t j = 0; j < cluster.count; ++j)
//    {
        const auto& hair = m_hairs[i];
        nvmath::vec3f p0 = transInv * hair.v0.p;
        nvmath::vec3f p1 = transInv * hair.v1.p;
        nvmath::vec3 extent = (nvmath::vec3(hair.thickness)) * (nvmath::vec3(1.0f) - nvmath::normalize(p1 - p0));
        nvmath::vec3 min = nvmath::nv_min(p0 - extent, p1 - extent);
        nvmath::vec3 max = nvmath::nv_max(p0 + extent, p1 + extent);
        aabb = Aabb{min, max};
        nvmath::vec3 diag = max - min;
        surface += diag.x * diag.y + diag.x * diag.z + diag.y * diag.z;
//    }

    // return surface of aabbs
    return surface;
}

void HelloVulkan::addCluster(std::vector<Aabb>& aabbs, nvmath::mat4& trans, Cluster& cluster, VkCommandBuffer& cmdBuf)
{
    m_trans.push_back(trans);
    m_clusters.push_back(cluster);
    m_clustersAabbBuffer.emplace_back(m_alloc.createBuffer(
            cmdBuf, aabbs, VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
                               VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR));
}

inline float getHalfCylinderSurface(const HelloVulkan::Hair& seg)
{
    return PI * seg.thickness * (seg.thickness + nvmath::length(seg.v1.p - seg.v0.p));
}
...
void HelloVulkan::createBottomLevelAS(std::ofstream& infoFile, std::ofstream& clusterLog, float minFillDegree)
{
    nvmath::vec3 max = m_hairs.back().v0.p;
    nvmath::vec3 min = m_hairs.back().v0.p;
    for (const auto& hair : m_hairs)
    {
        // min max of grid
        max = nvmath::nv_max(max, nvmath::nv_max(hair.v0.p, hair.v1.p));
        min = nvmath::nv_min(min, nvmath::nv_min(hair.v0.p, hair.v1.p));
    }
    max -= min;
    // keep one spare cell because the hair segments
    nvmath::vec3 scale_factor = nvmath::vec3(grid_size, grid_size, grid_size) / max;
    for (const auto& hair : m_hairs)
    {
        // change scale from min-max to 0-2048 and assign each hair to a cell of the grid
        nvmath::vec3 center = ((hair.v0.p - hair.v1.p) / 2.0f) + hair.v0.p;
        center -= min;
        center *= scale_factor;
        uint32_t x = static_cast<uint32_t>(std::floor(center.x));
        uint32_t y = static_cast<uint32_t>(std::floor(center.y));
        uint32_t z = static_cast<uint32_t>(std::floor(center.z));
        uint64_t morton = libmorton::m3D_e_LUT<uint64_t, uint32_t>(x, y, z);
        m_morton_hairs.emplace_back(std::pair(morton, hair));
    }
#if 1
    std::sort(m_morton_hairs.begin(), m_morton_hairs.end(), [](std::pair<uint64_t, Hair>& a, std::pair<uint64_t, Hair>& b)
    {
        return a.first < b.first;
    });
    m_hairs.clear();
    for (const auto& hair : m_morton_hairs)
    {
        m_hairs.emplace_back(hair.second);
    }
#endif
    ...
    allBlas.reserve(m_objInstance.size() + m_hairs.size());
    /*for(const auto& inst : m_objInstance)
    {
      auto blas = objectToVkGeometryKHR(m_objModel[inst.objIndex]);
    ...
    std::vector<Aabb> bestAabbs;
    for (uint32_t i = 0; i < m_hairs.size() - 1; ++i)
    {
        Hair& hair = m_hairs[i];
        Aabb aabb;
        nvmath::mat4 trans{};
        trans = calculateTransformation(cluster);
        calculateAabb(aabb, trans, i);
        bestAabbs.emplace_back(aabb);
        ++cluster.count;
        float bestFillDegree = 0.0f;
        uint32_t bestIndex = 0;
        for (uint32_t j = i + 1; j < i + 5 && j < m_hairs.size(); ++j)
        {
            Hair h = m_hairs[i + 1];
            m_hairs[i + 1] = m_hairs[j];
            trans = calculateTransformation(cluster);
            float surface = calculateAabb(aabb, trans, i + 1);
            ...
            float fillDegree = getHalfCylinderSurface(m_hairs[i + 1]) / surface;
            assert(fillDegree > 0.0f);
        ...
        if (bestFillDegree < minFillDegree)
        {
            ...
            --cluster.count;
            trans = calculateTransformation(cluster);
            addCluster(bestAabbs, trans, cluster, cmdBuf);
        ...
        {
            // swap best segment behind the current cluster and expand cluster
            Hair h = m_hairs[i + 1];
            m_hairs[i + 1] = m_hairs[bestIndex];
            m_hairs[bestIndex] = h;
            oldAllSegSur += getHalfCylinderSurface(m_hairs[i + 1]);
        }
    }
    ...
    Aabb aabb;
    trans = calculateTransformation(cluster);
    calculateAabb(aabb, trans, cluster.index + cluster.count - 1);
    bestAabbs.emplace_back(aabb);
    addCluster(bestAabbs, trans, cluster, cmdBuf);
    ...
    {
        allBlas.emplace_back(hairToVkGeometryKHR(i));
    }
...
void HelloVulkan::createRtPipeline()
{
    ...
    // Closest hit
    stage.module = nvvk::createShaderModule(m_device,
                                            nvh::loadFile("spv/raytraceHair.rchit.spv", true, defaultSearchPaths,
                                                          true));
    stage.stage = VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR;
    stages[eClosestHair] = stage;
    //Hair Intersection
    stage.module = nvvk::createShaderModule(m_device,
                                            nvh::loadFile("spv/raytrace.rint.spv", true, defaultSearchPaths, true));
    stage.stage = VK_SHADER_STAGE_INTERSECTION_BIT_KHR;
    stages[eHairIntersect] = stage;
    ...
    group.closestHitShader = eClosestHair;
    group.intersectionShader = eHairIntersect;
    m_rtShaderGroups.push_back(group);
